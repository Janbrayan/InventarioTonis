...
export default function Ventas() {
  ...
  // Modal ver detalles de una venta ya creada
  ...
  /**
   * Llamamos a "getSalesToday()" para ver solo las ventas del día, y a "getProducts"
   */
  async function fetchAll() {
    try {
      setLoading(true);
      const [ventasList, prodList] = await Promise.all([
        window.electronAPI.getSalesToday(), // Cambiado a getSalesToday
  ...
  // ===================== useEffect: Abrir modal + agregar producto si se llega con state =====================
  // Se ejecutará una vez que loading sea 'false' (es decir, ya cargamos productos)
  useEffect(() => {
    if (!loading) {
      ...
      if (st?.openModal === 'createSale') {
        ...
        handleOpenCreate();
        // Si hay un productId => agregarlo automáticamente
        if (typeof st.productId === 'number') {
          addProductById(st.productId);
        }
        // Limpia para no re-abrir al recargar
  ...
  // ===================== Eliminar renglón de la tabla interna =====================
  ...
  // ===================== Cambiar cantidad/precio en renglón =====================
  ...
  async function handleSaveVenta() {
    const action = async () => {
      try {
        const total = calcularTotal();
        // Armar objeto saleData con los renglones sin 'subtotal'
        const saleData = {
          total,
  ...
  // ===================== Dialogo de confirmación helpers =====================
  ...
  // ===================== Ver detalles de una venta existente =====================
    ...
    <Box sx={{ p: 3, width: '100%' }}>
      ...
      >
        ...
        <DialogContent sx={{ display: 'flex', flexDirection: 'column', gap: 2, mt: 1 }}>
          ...
          {/* Tabla interna de renglones de la venta */}
